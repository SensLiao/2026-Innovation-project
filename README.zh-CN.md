...
SOMA 是一个以临床医生为中心的胸部 CT 医学 AI 平台。它将一次扫描从上传一路带到一份签署的、带证据引用的放射学报告，并在每一步都让医生参与决策：AI 负责起草，放射科医生负责审阅、修改并批准。项目由四人团队构建 —— Ruixuan Liao、Mingzhe Cai、Siyu Chen、Kening Hu —— 并在 USYD Coding Fest 2026（School of Computer Science，2026 年 7 月 28 日）荣获 🏆 **Impactful Tech Award** 冠军。公开演示运行在合成数据之上：[app.soma-ai.org](https://app.soma-ai.org)。

<p align="center">
  <a href="https://app.soma-ai.org">在线演示</a> ·
  <a href="https://soma-ai.org">官方网站</a>
</p>
...
整个工作流在设计上即为 human-in-the-loop。一张 CT 切片及其分割掩膜进入流水线；四个专家智能体依次接力 —— Radiologist 交给 Pathologist，再交给 Report Writer —— 最后由 QC Reviewer 按六个评分类别为草稿把关。每个智能体都从同一个精选知识库中检索，因此 findings、differential 与最终措辞引用的是同一批指南条目。医生是流程的最后一环，而不是旁观者：
...
底层系统是一条从浏览器直达编排器的清晰路径。React SPA 与 Node.js + Express 5 API 通信，后者负责认证与病例状态，并把每一个临床步骤都委派给多智能体编排器。再往下是 AI/ML 服务 —— 负责分割的 SAM-Med2D、通过 AWS Bedrock 驱动专家智能体的 Claude、以及提供 PubMedBERT 嵌入的 FastAPI sidecar —— 以及数据层：带 pgvector 的 PostgreSQL，存放病例、报告、掩膜与指南索引。整个技术栈通过容器化的 CI/CD 流水线交付。
...
SOMA 是一个研究原型，**仅供研究与教育用途** —— 它**不是医疗器械，也不用于临床**。公开演示运行在合成数据之上。
